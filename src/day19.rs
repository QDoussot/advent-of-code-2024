...
#[derive(Default)]
struct PTree(bool, HashMap<u8, PTree>);

fn add_to_tree(pattern: &[u8], PTree(leaf, child): &mut PTree) {
    match pattern {
        [] => *leaf = true,
        [h, t @ ..] => {
...
fn build_pattern_tree(availables: &[Pattern]) -> PTree {
    let mut tree = PTree::default();
...
fn possible_next_end(pattern: &[u8], PTree(leaf, child): &PTree, depth: usize) -> Vec<usize> {
    ...
    [sub, if *leaf { vec![depth] } else { vec![] }].concat()
}

fn possible(pattern: &[u8], availables: &PTree) -> bool {
...
fn possibles_count(pat: &[u8], availables: &PTree) -> usize {
